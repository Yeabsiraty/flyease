...
export function SuccessBanner() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("booking-success", onEvt);
    return () => window.removeEventListener("booking-success", onEvt);
  }, [pathname]);
  ...
  return (
    <div role="status" className="sticky top-16 z-30 border-b border-gold/40 bg-gold/10 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2 text-sm font-medium text-foreground">
          <CheckCircle2 className="h-5 w-5 text-gold" />
          <span>Your message has been received — we will reach you soon on WhatsApp.</span>
        </div>
        <button
          aria-label="Dismiss"
          onClick={dismiss}
